...
export const handler = async (request: RegisterRequest): Promise<LoginResponse> => {
    ...
    const [userDto, authTokenDto] = await registerService.register(request.firstName, request.lastName, request.userAlias, request.password, request.userImageBytes, request.imageFileExtension)
    return {
        success: true,
        message: null,
        user: userDto,
